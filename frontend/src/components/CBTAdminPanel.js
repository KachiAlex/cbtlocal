...
// Improved parsing functions from original CBT app

// Excel parsing function - improved version from original CBT app
async function parseQuestionsFromExcel(file) {
  try {
    console.log('ðŸ” Starting Excel import for file:', file.name);
    ...
    console.log('ðŸ“‹ Available worksheets:', worksheetNames);
    ...
    for (const name of possibleNames) {
      ...
      if (worksheet) {
        console.log('âœ… Found worksheet:', name);
        break;
      }
    }
    ...
    if (!worksheet) {
      ...
      if (worksheet) {
        console.log('âœ… Using first worksheet:', worksheet.name);
      }
    }
    ...
    console.log('ðŸ“Š Worksheet info:', {
      name: worksheet.name,
    ...
    if (firstCellValue.includes('question') || firstCellValue.includes('option')) {
      startRow = 2;
      console.log('ðŸ“ Detected header row, starting from row 2');
    }
    ...
    for (let rowNumber = startRow; rowNumber <= worksheet.rowCount; rowNumber++) {
      ...
      if (!questionText || questionText.length < 10) {
        console.log(`â­ï¸ Skipping row ${rowNumber} - no question text`);
        continue;
      }
      
      // Check if we have all required fields
      if (!optionA || !optionB || !optionC || !optionD || !correctAnswer) {
        console.log(`â­ï¸ Skipping row ${rowNumber} - missing options or answer`);
        continue;
      }
      ...
      if (answerIndex === undefined) {
        console.log(`â­ï¸ Skipping row ${rowNumber} - invalid answer: ${correctAnswer}`);
        continue;
      }
      ...
      console.log(`âœ… Added question ${questions.length}: ${questionText.substring(0, 50)}...`);
    }
    
    console.log(`ðŸŽ‰ Successfully parsed ${questions.length} questions from Excel file`);
    ...
    console.error('âŒ Error parsing Excel file:', error);
...
// CSV parsing function - simplified version
function parseQuestionsFromCSV(csvText) {
  ...
  // Find column indices
  const questionCol = headerRow.findIndex(h => h.includes('question'));
  const optionACol = headerRow.findIndex(h => h.includes('option a') || h.includes('a)'));
  const optionBCol = headerRow.findIndex(h => h.includes('option b') || h.includes('b)'));
  const optionCCol = headerRow.findIndex(h => h.includes('option c') || h.includes('c)'));
  const optionDCol = headerRow.findIndex(h => h.includes('option d') || h.includes('d)'));
  const answerCol = headerRow.findIndex(h => h.includes('answer') || h.includes('correct'));
  
  if (questionCol === -1 || optionACol === -1 || optionBCol === -1 || optionCCol === -1 || optionDCol === -1 || answerCol === -1) {
    throw new Error('CSV format not recognized. Expected columns: Question, Option A, Option B, Option C, Option D, Answer');
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (row.length < Math.max(questionCol, optionACol, optionBCol, optionCCol, optionDCol, answerCol) + 1) {
      continue; // Skip incomplete rows
    }
    
    const questionText = row[questionCol];
    const optionA = row[optionACol];
    const optionB = row[optionBCol];
    const optionC = row[optionCCol];
    const optionD = row[optionDCol];
    const correctAnswer = row[answerCol];
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      if (fileExtension === 'docx') {
        ...
        console.log('ðŸ” Parsed questions:', parsed);
        ...
        setQuestions(parsed);
        if (selectedExam) {
          console.log('ðŸ” Saving questions for exam:', selectedExam.id);
          saveQuestionsForExam(selectedExam.id, parsed);
          updateExamQuestionCount(selectedExam.id, parsed.length);
        } else {
        ...
        console.log('ðŸ” Starting CSV import for file:', file.name);
      ...
      <main className="max-w-7xl mx-auto w-full px-3 sm:px-8 py-4 sm:py-8">
            ...
            <div>
              ...
              {activeTab === "questions" && selectedExam && (
                <div className="mt-2 text-sm text-blue-600 bg-blue-50 px-3 py-2 rounded-lg border border-blue-200">
                  ðŸ“ Currently viewing questions for: <span className="font-semibold">{selectedExam.title}</span>
                </div>
              )}
            </div>
        ...
        <div className="flex space-x-1 bg-gray-100 p-1 rounded-xl mb-8">
          {(() => {
            ...
            { id: "settings", label: "âš™ï¸ Settings", icon: "âš™ï¸", adminOnly: true },
            { id: "advanced-exams", label: "ðŸ”§ Advanced Exams", icon: "ðŸ”§", adminOnly: true }
            ];
            
            console.log('ðŸ” DEBUG: User object:', user);
            console.log('ðŸ” DEBUG: User role:', user?.role);
            console.log('ðŸ” DEBUG: All tabs:', allTabs.map(t => ({ id: t.id, adminOnly: t.adminOnly })));
            ...
            const visibleTabs = allTabs.filter(tab => {
              ...
              console.log(`ðŸ” Tab ${tab.id}: adminOnly=${tab.adminOnly}, shouldShow=${shouldShow}`);
              return shouldShow;
            });
            
            console.log('ðŸ” DEBUG: Visible tabs:', visibleTabs.map(t => t.id));
              ...
              <div key={exam.id} className={`border rounded-xl p-4 transition-all ${
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    >
                      {selectedExam?.id === exam.id ? 'ðŸ‘ï¸ Viewing Questions' : 'ðŸ“ View Questions'}
                    </button>
  ...
  if (!selectedExam) {
    return (
      <div className="text-center py-8 text-gray-500">
        ...
        >
          â† Go to Exam Management
        </button>
        <div className="mt-6 text-sm text-gray-500">
          ðŸ’¡ Tip: Click the "ðŸ“ View Questions" button on any exam to see its questions
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <h3 className="text-lg font-semibold text-blue-900">ðŸ“ Questions for: {selectedExam.title}</h3>
            ...
            <div className="flex flex-wrap gap-4 mt-2 text-xs text-blue-600">
              <span>ðŸ“Š Questions: {questions.length}</span>
              <span>â±ï¸ Duration: {selectedExam.duration} minutes</span>
              <span>ðŸ“… Created: {new Date(selectedExam.createdAt).toLocaleDateString()}</span>
              {selectedExam.isActive && <span className="px-2 py-0.5 rounded-full bg-green-100 text-green-700">ðŸŸ¢ Active</span>}
            </div>
        </div>
        <button
          onClick={onBackToExams}
      ...
      <div className="bg-white rounded-xl border p-6">
        <div className="flex justify-between items-center mb-4">
          <h4 className="font-semibold text-lg">ðŸ“‹ Exam Questions ({questions.length})</h4>
          {questions.length > 0 && (
            <div className="text-sm text-gray-500">
          ...
          <div className="text-center py-8 text-gray-500">
            ...
            {isAdmin && (
              <div className="text-xs text-blue-600">
                ðŸ’¡ Tip: Use the upload section above to add questions from Word or Excel files
              </div>
            )}
          </div>
        ) : (
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <span className="text-indigo-600 font-semibold">ðŸ“ Upload Logo File</span>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <span className="text-gray-600">ðŸ”— Logo URL</span>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-2xl mx-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setAdminsModalOpen(false)} className="text-gray-500 hover:text-gray-700 text-xl">Ã—</button>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-2xl mx-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setStudentsModalOpen(false)} className="text-gray-500 hover:text-gray-700 text-xl">Ã—</button>
            </div>
            <div className="text-center text-gray-500 py-8">
...
// Duplicate function removed - using the first CreateExamTab definition above
