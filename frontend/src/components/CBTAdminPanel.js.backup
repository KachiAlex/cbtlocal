  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        console.log('ðŸ” Detected column structure:', columnMap);
        ...
        console.log(`âœ… Successfully parsed ${questions.length} questions from Excel`);
        resolve(questions);
      } catch (error) {
        reject(error);
      }
...
const parseQuestionsFromMarkdown = (markdown) => {
  ...
  console.log(`âœ… Successfully parsed ${questions.length} questions from Markdown`);
  return questions;
};
...
const parseQuestionsFromCSV = (csvText) => {
  ...
  console.log('ðŸ” Detected CSV delimiter:', delimiter);
  
  // Parse header row
  const headerRow = parseCSVLine(lines[0], delimiter);
  const columnMap = detectCSVColumnStructure(headerRow);
  
  console.log('ðŸ” Detected CSV column structure:', columnMap);
  ...
  console.log(`âœ… Successfully parsed ${questions.length} questions from CSV`);
  return questions;
};
